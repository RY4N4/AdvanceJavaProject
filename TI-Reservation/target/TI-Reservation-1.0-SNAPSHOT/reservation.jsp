...
<%
    ...
    int travelers = (travelersStr != null && !travelersStr.isEmpty()) ? Integer.parseInt(travelersStr) : 1;

    boolean formSubmitted = tripType != null || tripFrom != null || tripTo != null;
    if (formSubmitted) {
%>
...
<div class="search-results">
    ...
    <table>
        ...
        <tbody>
        <%
            String url = "jdbc:mysql://localhost:3306/Travel-Itinerary";
            String user = "root";
            String password = "";
            Connection connection = null;
            PreparedStatement psmt = null;
            ...
            try {
                ...
                String query = "SELECT * FROM trips WHERE 1=1";
                if (tripType != null) query += " AND Trip_Type = ?";
                if (tripFrom != null) query += " AND Trip_From LIKE ?";
                if (tripTo != null) query += " AND Trip_To LIKE ?";
                if (startDate != null) query += " AND Trip_StartDate >= ?";
                if (endDate != null) query += " AND Trip_EndDate <= ?";

                psmt = connection.prepareStatement(query);
                int i = 1;
                if (tripType != null) psmt.setString(i++, tripType);
                if (tripFrom != null) psmt.setString(i++, "%" + tripFrom + "%");
                if (tripTo != null) psmt.setString(i++, "%" + tripTo + "%");
                if (startDate != null) psmt.setDate(i++, java.sql.Date.valueOf(startDate));
                if (endDate != null) psmt.setDate(i++, java.sql.Date.valueOf(endDate));
                ...
                while (rs.next()) {
                    hasResults = true;
                    int tripId = rs.getInt("Trip_ID");
                    double pricePerTraveler = rs.getDouble("Trip_Price");
                    double totalPrice = pricePerTraveler * travelers;
        %>
        <tr>
            <td><%= tripId %></td>
            <td><%= rs.getString("Trip_From") %></td>
            ...
            <td><%= rs.getDate("Trip_StartDate") %></td>
            <td><%= rs.getDate("Trip_EndDate") %></td>
            <td>$<%= pricePerTraveler %></td>
            <td><strong>$<%= totalPrice %></strong></td>
            <td>
                <form action="package-details.jsp" method="post">
                    <input type="hidden" name="tripId" value="<%= tripId %>">
                    <input type="hidden" name="totalPrice" value="<%= totalPrice %>">
                    <button type="submit" class="action-button">Proceed with Package</button>
                </form>
            </td>
        </tr>
        <%
        ...
        %>
        <tr>
            <td colspan="9">No matching trips found. Please refine your search.</td>
        </tr>
        <%
            }
        } catch (Exception e) {
